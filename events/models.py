...
class Event(models.Model):
    ...
    location_meeting_link = models.URLField(blank=True, null=True)  # for online/hybrid events
    ...
    special_requirements = models.TextField(blank=True, null=True)  # Accessibility, technical setup
    equipment_needed = models.TextField(blank=True, null=True)  # e.g., projector, mic, etc.
...
class EventParticipant(models.Model):
    ...
    user = models.ForeignKey(User, related_name='registered_events', on_delete=models.CASCADE)
...
class EventInvitation(models.Model):
    INVITATION_TYPE_CHOICES = [
        ('guest', 'Guest'),
        ('facilitator', 'Facilitator'),  # Facilitators could be speakers, trainers, etc.
    ]
...
class EventFacilitator(models.Model):
    event = models.ForeignKey(Event, related_name='facilitators', on_delete=models.CASCADE)
    facilitator = models.ForeignKey(User, related_name='facilitated_events', on_delete=models.CASCADE)
    role = models.CharField(max_length=255)  # e.g., Speaker, Trainer, etc.
    confirmed = models.BooleanField(default=False)  # Facilitators can confirm their participation
