...
class EventBasicInfoFormTest(TestCase):
    ...
    def test_missing_description(self):
        form = EventBasicInfoForm(
            data={"title": "Sample Event", "event_type": "meeting", "mode": "online"}
        )
        self.assertFalse(form.is_valid())
...
class EventLocationFormTest(TestCase):
    ...
    def test_physical_address_required_for_in_person_mode(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertIn(
            "Physical address can't be empty.", form.errors["physical_address"]
        )
...
class EventRequirementsFormTest(TestCase):
    ...
    def test_missing_special_requirements(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertIn(
            "Special requirements can't be empty.", form.errors["special_requirements"]
        )
    ...
    def test_missing_equipment_needed(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertIn(
            "Equipment needed can't be empty.", form.errors["equipment_needed"]
        )
